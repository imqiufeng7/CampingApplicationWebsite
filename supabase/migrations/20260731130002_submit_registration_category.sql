-- Extends fn_submit_registration with optional per-category capacity/member-cap
-- enforcement and the is_free -> automatic 無需繳費 override. When a session has no
-- session_registration_categories rows, behavior is byte-for-byte identical to before
-- (falls through to the existing session-wide capacity_total/max_members_per_registration
-- checks).
...
begin
  ...
  insert into public.registrations (session_id, registration_category_id, contact_email, contact_phone)
  values (v_session.id, v_registration_category_id, payload ->> 'contact_email', payload ->> 'contact_phone')
