-- Registrant-facing survey field: whether the group wants to borrow a 淨斯福慧床
-- (Jing Si foldable bed) for the overnight camp-out. Registration-level (one answer
-- per registration, not per member) — same table as contact_email/contact_phone.
-- Same lightweight design as existing enum-ish columns like registration_members'
-- gender/meal_diet — plain text, no DB check constraint, validated at the Zod/app
-- layer only. Existing rows default to '不需要' since the question didn't exist when
-- they were submitted.
alter table public.registrations
  add column comfort_bed_needed text not null default '不需要';

-- fn_submit_registration: identical to the version in
-- 20260901090001_min_members_per_registration.sql except the registrations insert
-- now also carries comfort_bed_needed.
...
begin
  ...
  insert into public.registrations (
    session_id, registration_category_id, contact_email, contact_phone, comfort_bed_needed, consent_given_at
  )
  values (
    v_session.id, v_registration_category_id, payload ->> 'contact_email', payload ->> 'contact_phone',
    coalesce(nullif(payload ->> 'comfort_bed_needed', ''), '不需要'), now()
  )
...
-- fn_get_registration_for_edit: identical to 20260903090001_edit_link_lock_and_lookup_by_phone.sql's
-- version except the top-level jsonb now also includes comfort_bed_needed.
...
begin
  ...
  select jsonb_build_object(
    ...
    'comfort_bed_needed', v_registration.comfort_bed_needed,
...
-- fn_update_registration_via_token: identical to
-- 20260903090001_edit_link_lock_and_lookup_by_phone.sql's version except the
-- registrations update now also carries comfort_bed_needed.
