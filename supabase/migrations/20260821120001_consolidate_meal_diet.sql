-- Consolidates 20260821110001's two separate 宵夜/早餐 diet columns into one shared
-- choice — the vendor found picking twice for what's almost always the same answer
-- more annoying than useful. No real registrations have used either column yet
-- (confirmed via direct query before writing this), so this drops and replaces
-- rather than migrating data.
alter table public.registration_members
  drop column midnight_snack_diet,
  drop column breakfast_diet,
  add column meal_diet text;
