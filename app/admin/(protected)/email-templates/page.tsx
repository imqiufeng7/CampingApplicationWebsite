...
const TYPE_LABELS: Record<EmailType, string> = {
  報名確認: "報名確認信（送出報名後立即寄出，含查詢連結）",
  審核結果: "審核結果/繳費通知（舊版，不再使用）",
  "審核結果-正取": "錄取通知/繳費通知（正取，後台批次寄送）",
  "審核結果-備取": "備取通知（備取，後台批次寄送）",
  退回補件: "需補件通知（審核設為「退回補件」時寄出，含修改連結）",
  開放修改: "開放修改通知（後台手動「允許再修改一次」時寄出，含修改連結）",
  付款通知: "付款通知",
  場次資訊: "場次資訊",
  報到QR: "報到 QR",
  遞補通知: "遞補通知",
...
const SAMPLE_VARS: Partial<Record<EmailType, Record<string, string>>> = {
  報名確認: {
    ...
    第一位成員姓名: "王小明",
    成員名單: "1. 王小明\n2. 陳小華",
    查詢連結: "https://example.com/lookup?email=example@mail.com&phone=0912345678",
  },
  "審核結果-正取": {
    活動名稱: "2026年防災教育營",
    ...
    錄取結果: "您已錄取本次活動（正取）。",
    成員審核結果:
      "聯絡人(成員1)：李瑩蓁，符合免付費申請資格，無需繳交報名費\n成員2：邱千芸，符合免付費申請資格，無需繳交報名費\n成員3：邱千芳，未符合免付費申請資格，需繳報名費150元\n成員4：邱國達，未申請免付費申請資格，需繳報名費150元\n您需要繳交的團隊報名費共計300元。",
    繳費資訊: "繳費連結：https://example.com/pay/abc123\n繳費期限：114年10月31日（星期五）12:00 前",
  },
  "審核結果-備取": {
    活動名稱: "2026年防災教育營",
    活動日期: "11月15日(星期六)至11月16日(星期日)",
    第一位成員姓名: "李瑩蓁",
    錄取結果: "您目前為備取名單，若有名額釋出將另行通知。",
    成員審核結果:
      "聯絡人(成員1)：李瑩蓁，符合免付費申請資格，無需繳交報名費\n成員2：邱千芸，未符合免付費申請資格，需繳報名費150元\n您需要繳交的團隊報名費共計150元。",
